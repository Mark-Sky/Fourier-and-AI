...
def plot_decision_boundary_sklearn(data_x, data_y, w, b, r_features, clf, boundary=False, title="", path=None):
    ...
    if boundary:
        ...
        data_xx = np.c_[xx1.ravel(), xx2.ravel()]
        XX = np.sqrt(2 / r_features) * np.cos(data_xx @ w + b)
        pred_yy = clf.predict(XX)
        plt.contourf(xx1, xx2, pred_yy.reshape(xx1.shape), alpha=0.4)
